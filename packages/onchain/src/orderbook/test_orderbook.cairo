...
fn test_lock_inscription_fails_prior_lock_not_expired() {
    ...
    orderbook_dispatcher.lock_inscription(id, "hash");
    let (_, hash, inscription_block) = orderbook_dispatcher.query_inscription_lock(id);
    assert_eq!(hash, "hash");
...
fn test_submit_inscription_works() {
    ...
    orderbook_dispatcher.lock_inscription(id, tx_hash.clone());

    let (_, hash, _) = orderbook_dispatcher.query_inscription_lock(id);
    ...
    orderbook_dispatcher.submit_inscription(id, tx_hash, tx, 868239, block_header, inclusion_proof);
}
